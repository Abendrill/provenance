...
/// A lightweight key referencing a value stored in a [ProvenanceMap](crate::ProvenanceMap) or
/// [SeparateProvenanceMap](crate::SeparateProvenanceMap).
///
...
/// it with its provenance. And since there only may be one map with any given
/// provenance, it is guaranteed that if a key matches the required type signature
/// for retrieving a value from a map, then that key was created by that map and
/// references a value in that map.
